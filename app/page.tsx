...
export default function DriverDashboard() {
  ...
  const [isCashQROpen, setIsCashQROpen] = useState(false) // Added state for CashQRDialog
  const [showQRScannerDialog, setShowQRScannerDialog] = useState(false) // New state for QR scanner dialog visibility
  ...
  const handleAcceptQueueQR = (passengerId: number) => {
    ...
    console.log("[v0] accept:success", {
      passengerId: passengerId,
      seatsOccupied: passengerCount,
      timestamp: new Date().toISOString(),
    })
  ...
  const handleScanQueueQR = () => {
    if (areSeatsLocked) {
      ...
      toast({
        title: language === "ru" ? "Заблокировано" : "Blocked",
        description: language === "ru" ? "Сначала начните подготовку рейса" : "Start trip preparation first",
        variant: "destructive",
      })
      return
    ...
    if (scanInProgressRef.current || isScanningLocked) {
      return
    }
    ...
    if (!nextPassenger) {
      toast({
        title: language === "ru" ? "Нет пассажиров" : "No passengers",
        description: language === "ru" ? "Все пассажиры уже обработаны" : "All passengers processed",
      })
      return
    }
    ...
    setIsScanningLocked(true)
    setCurrentQueueScanId(nextPassenger.id)
    setShowQRScannerDialog(true) // Use the new state variable
  ...
  const handleQueuePassengerScan = (qrResult: {
    ...
    if (qrResult.match) {
      ...
      updatedPassengers[passengerIndex] = {
        ...
        qrError: false,
        qrData: {
          sum: qrResult.sum || 450,
          recipient: qrResult.recipient || "driver_247",
          created_at:
            qrResult.created_at ||
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-2 sm:p-4">
      ...
      <div className="px-2 pt-4 space-y-6">
        ...
        {userStatus === "confirmed" && tripStatus !== STATE.IN_ROUTE && (
          <Card className={`p-4 border-2 border-border ${isPanelsDisabled ? "opacity-50 pointer-events-none" : ""}`}>
            ...
            <div className="grid grid-cols-5 gap-2 mb-4">
              {queuePassengers.slice(0, 4).map((passenger) => (
                <div
                  ...
                  data-booking-id={passenger.id}
                >
                  {qrScannedData && qrScannedData.scannedPassengerId === passenger.id ? (
                    // Show sum and accept/reject buttons after successful scan
                    <div className="flex flex-col items-center justify-center w-full h-full gap-1">
                      <span className="text-sm font-bold">{formatCurrency(qrScannedData.amount)}</span>
                      <div className="flex gap-1">
                        <Button
                          onClick={() => handleAcceptQueueQR(passenger.id)}
                          size="sm"
                        ...
                        <Button
                          onClick={() => handleRejectQueueQR(passenger.id)}
                          size="sm"
                          variant="destructive"
                          className="h-6 px-2 text-xs"
                          disabled={isPanelsDisabled}
              ...
              {queuePassengers.length >= 5 && (
                <div
                  ...
                  className={`h-20 flex flex-col items-center justify-center p-2 rounded-md border-2 border-dashed ${
                    queuePassengers[4].qrError
                      ? "bg-red-100 border-red-500 dark:bg-red-900/30 dark:border-red-600"
                      : queuePassengers[4].scanned
                        ? "bg-green-100 border-green-500 dark:bg-green-900/30 dark:border-green-600"
                        : "bg-secondary border-border"
                  }`}
                  data-booking-id={queuePassengers[4].id}
                >
                  {qrScannedData && qrScannedData.scannedPassengerId === queuePassengers[4].id ? (
                    // Show sum and accept/reject buttons after successful scan
                    <div className="flex flex-col items-center justify-center w-full h-full gap-1">
                      <span className="text-sm font-bold">{formatCurrency(qrScannedData.sum)}</span>
                      <div className="flex gap-1">
                        <Button
                          onClick={() => handleAcceptQueueQR(queuePassengers[4].id)}
                          size="sm"
                          className="h-6 px-2 text-xs"
                          disabled={isPanelsDisabled}
                        >
                        ...
                        <Button
                          onClick={() => handleRejectQueueQR(queuePassengers[4].id)}
                          size="sm"
                          variant="destructive"
                          className="h-6 px-2 text-xs"
                          disabled={isPanelsDisabled}
            ...
            {/* Removed qrScannedData card - buttons now show inside passenger cards */}
      ...
      <CashQRDialog
        open={isCashQROpen} // Use the new state variable
        onOpenChange={(isOpen) => {
          setIsCashQROpen(isOpen) // Update the new state variable
          if (!isOpen) {
      ...
      {/* Added CashQRDialog here */}
      <CashQRDialog
        open={showQRScannerDialog}
        onOpenChange={(isOpen) => {
          setShowQRScannerDialog(isOpen)
          if (!isOpen) {
